...
function ArchiveImg() {
    ...
    return (
        <svg
            viewBox="79.164 91.172 63.675 66.68"
            width="1.5em"
            height="1.5em"
        ...
        >
            <path
                style={style}
                d="M 89.898 99.084 H 131.124 A 4.5 4.467 0 0 1 135.624 103.551 V 106.018 A 2 2 0 0 1 133.624 108.018 H 87.398 A 2 2 0 0 1 85.398 106.018 V 103.551 A 4.5 4.467 0 0 1 89.898 99.084 Z"
                bxShape="rect 85.398 99.084 50.226 8.934 4.5 4.5 2 2 1@72b6ad5a"
            />
            <rect
                x="90.259"
                y="113.324"
...
function MatchTableRow({
    ...
    return (
        ...
        >
            ...
            <td style={styleSub(subA)}>
                {subA.path}
                {subA.isArchive && <ArchiveImg />}
            </td>
            <td style={styleSub(subB)}>
                {subB.path}
                {subB.isArchive && <ArchiveImg />}
            </td>
...
const defaultMatchTableCallbacks = {
    mouseenter: ({ submissionA = null, submissionB = null, group = -1 }) =>
        null,
    mouseleave: (event) => null,
};
...
function MatchTable({
    ...
    class MatchTableRowSubmission {
        constructor(node) {
            ...
            this.isHighlighted =
                ...
                highlighted.nodes.includes(node.id);
        }
    }
    ...
    const rows = graph.links
        .filter((link) => {
            ...
            return (
                link.value >= cutoff &&
                (selected === null || selected.group === group)
            );
        })
        .map((link) => {
            ...
            <thead>
                <tr>
                    ...
                    <th colSpan="2">
                        Score{" "}
                        <span
                            data-tip="Similarity of submissions on a relative scale from 1 to 10."
